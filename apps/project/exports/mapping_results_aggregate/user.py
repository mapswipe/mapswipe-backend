...
def get_disagreeing_contributions_per_user_and_task(row: pd.Series) -> int:
    ...
    return total_count - (agreeing_contributions + 1)


def generate_mapping_results_aggregate_by_user(
    *,
    ...
    agg_results_by_user_id_df = raw_contributions_df.groupby(
        ...
        agreeing_contributions=pd.NamedAgg(
            column="agreeing_contributions",
            aggfunc="sum",
        ),
        disagreeing_contributions=pd.NamedAgg(
            column="disagreeing_contributions",
            aggfunc="sum",
        ),
    )

    # Calc simple agreement score as share of agreeing contributions.
    agg_results_by_user_id_df["simple_agreement_score"] = agg_results_by_user_id_df["agreeing_contributions"] / (
        agg_results_by_user_id_df["agreeing_contributions"] + agg_results_by_user_id_df["disagreeing_contributions"]
    )
