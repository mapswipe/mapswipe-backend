...
def get_progress_by_date(results_df: pd.DataFrame, groups_df: pd.DataFrame) -> pd.DataFrame:
    ...
    results_by_group_id_df["number_of_results"] = (
        results_by_group_id_df["number_of_users"] * results_by_group_id_df["number_of_tasks"]
    )
    results_by_group_id_df["cum_number_of_users"] = (
        results_by_group_id_df["number_of_users"].groupby(["project_id_x", "group_id"]).cumsum()
    )
...
def get_contributors_by_date(results_df: pd.DataFrame) -> pd.DataFrame:
    ...
    results_by_user_id_df["new_user"] = results_by_user_id_df.apply(
        lambda row: is_new_user(row["day"], row["first_day"]),
        axis=1,
    )
