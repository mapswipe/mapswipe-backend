...
class TestProjectAoiGeometry(TestCase):
    ...
    def test_deleting_project_keeps_other_projects_geometry(self):
        project = self.create_project_with_aoi_geometry()
        other_project = self.create_project_with_aoi_geometry()

        project.delete()

        other_project.refresh_from_db()
        assert other_project.aoi_geometry_id is not None
        assert list(Geometry.objects.values_list("pk", flat=True)) == [other_project.aoi_geometry_id]

    def test_deleting_project_queryset_keeps_other_projects_geometry(self):
        project = self.create_project_with_aoi_geometry()
        other_project = self.create_project_with_aoi_geometry()
